...
public class PathingMain extends PApplet
{
   ...
   private boolean depthFirstSearch(Point pos, GridValues[][] grid, List<Point> path)
   {
      ...
      if (withinBounds(pos, grid) && grid[pos.y][pos.x] != GridValues.OBSTACLE && grid[pos.y][pos.x] != GridValues.SEARCHED) {
         ...
         //check if p_i node is goal
         if (grid[pos.y][pos.x] == GridValues.GOAL) {
            currentValid = true;

         }
         //if not search new point
         else {
            ...
            currentValid = depthFirstSearch(rightN, grid, path) ||  depthFirstSearch(leftN, grid, path) ||  depthFirstSearch(downN, grid, path) || depthFirstSearch(upN, grid, path);
         }

      }
      //if not an osbtactle or searched node
